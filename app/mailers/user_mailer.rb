...
class UserMailer < ActionMailer::Base
  ...
  def welcome_email(user)
   ...
   mail(:to => email_with_name, :bcc => "user@taxizi.ru", :subject => "Taxi Zi регистрация #{@user.name} на сайте прошла успешна")
  end
  ...
  def activate_contract_email(contract)
    ...
    mail(to: email_with_name, bcc: "contract@taxizi.ru", subject: "Подтвердите свой заказ #{@contract.t_car} на Taxi Zi")
  end
  ...
  def new_offer_email(offer)
    ...
    mail(to: email_with_name, bcc: "offer@taxizi.ru", subject: "Ваш заказ на Taxi Zi получил новое предложение")
  end
  ...
  def offer_taken_email(offer)
    ...
    mail(to: email_with_name, cc: email_with_name2, bcc: "deal@taxizi.ru", subject: "Предложение на заказ на сайте Taxi Zi принято")
  end

  def newpass_email(user)
    @user = user
    email_with_name = "#{@user.name} <#{@user.email}>"
    mail(to: email_with_name, bcc: "user@taxizi.ru", subject: "Новый пароль #{@user.name} на сайте Taxi Zi.")
  end

  def posts_email(user)
    @user = user
    mail(to: "help@taxizi.ru", subject: "Post from Contact or Help page")
  end
